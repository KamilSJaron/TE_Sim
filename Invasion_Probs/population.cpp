...
bool Population::isExtinct(int fam)
{
    
    for(int i=0;i<popSize;i++){
        FamilyCensus * hed = GetIndividual(i).GetHeadFamily();
        
        while((hed!=0)&&(hed->GetFamily()<fam)){
            hed=hed->GetNext();
        }
...
double Population::GetPopulationMeanFamilyCount()
{
    double tbr = 0.0;
    for (int i=0; i < popSize; i++)
        tbr +=  GetIndividual(i).GetNumberOfFamilies();
    tbr/=popSize;
    return tbr;
...
void Population::Initialize(bool clonal, bool fromFile)
{
	...
	if (fromFile)
	{
        std::cout<<"This don't work, and I ain't fixing it today.\n";
        /*
	...
	*/
	...
	if (clonal)
	{
	  ...
	  for (int j=0; j < individualNumberOfTEs; j++)
	  {
		...
		GetIndividual(0).GetChromosome(num,copy).Insert(Transposon(pos, true,0));//clonal does not work for famililies, at present- they are all the same
	  }
	...
	else // not clonal
	{
	  ...
	  for (int i=0; i < popSize; i++)
	  {
        ...
        for(int whichFamily = 1;whichFamily<=(n);whichFamily++){
            
            individualNumberOfTEs = (int)rand.Poisson(29);
            ...
            for (int j=0; j < individualNumberOfTEs; j++)
            {
                ...
                GetIndividual(i).Insert(num,copy,Transposon(pos, affectW, whichFamily));
                
            }//for Poisson(29) times
...
void Population::DeleteIndividual(int x)
{
    ...
    delete genoVector.at(x).GetHeadFamily();
    genoVector.at(x).SetHeadFamily(0);
    genoVector.at(x).ResetTotalTECount();
		...
		while (p <= 2)
		{
		  ...
		  for (int i=1; i <= chr; i++)
		  {
			...
			while (current != 0)
			{
              ...
              newPopulation->GetIndividual(a).Insert(i,p,current->GetTransposon());
			  current = current->GetNext();
			}
		  } // for
...
//	std::ofstream fout(fileName,std::ios::app);
	...
	std::cout << "Initial number of families at equilibrium = " << Genome::initialTE << std::endl;
...
void Population::RecordPopulation(char fileName[], int generation)
{
    ...
    for (int i=0; i < size; i++)
    {
        fout<<"Individual #"<<i<<" has "<<GetIndividual(i).GetGenomeTECount()<<" TEs:";
        if (GetIndividual(i).GetGenomeTECount()==0)
            fout << "\n";
        else
        {
            for (int j =1; j <= chromNumber; j++){
                ...
                for (int k=1; k <= ploidy; k++)
                {
                    ...
                    while (loc != 0)
                    {
                        fout << "(" << loc->GetTransposon().GetLocation() <<","<<loc->GetTransposon().GetFamily()<<"), ";
                        loc = loc->GetNext();
                    }
                    if (j==chromNumber && k==ploidy)
                        fout<<"\n\n";
...
void Population::FamilyStats(int lastFam)
{
    
    for(int s=0;s<lastFam;s++){
        std::cout<<"Family "<<s<<": ";
        int howMany=0;
        for(int i =0;i<popSize;i++){
            FamilyCensus * counter = GetIndividual(i).GetHeadFamily();
            while((counter!=0)&&(counter->GetFamily()!=s)){
                counter = counter->GetNext();
            }
            if(counter!=0){
                howMany+=counter->GetCount();
            }
...
void Population::IntroduceElement(int fam)//introduces an element of family fam into a random location of a random individual
{
    int individualNumberOfTEs=0, num=0, copy=0, pos=0, totalLength=0, currentLength=0;
    int numberOfChromosomes = Genome::GetNumberOfChromosomes();
    int genomePloidy = Genome::GetPloidy();
    ...
    int i = (int)((rand.Uniform())*(popSize));
    do {
        pos = (int)((rand.Uniform()*totalLength) + 1);
        num = 1;
        for (int k=1; k <= numberOfChromosomes; k++)
    ...
    //GetIndividual(i).GetChromosome(num,copy).Insert(Transposon(pos, affectW,fam));
    GetIndividual(i).Insert(num,copy,Transposon(pos,affectW,fam));
